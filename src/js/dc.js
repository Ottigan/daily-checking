  ...
  populateRows = function (rowObjects) {
    ...
    do {
      if (i === 0) {
        ...
        document.querySelector(`#counter-${i}`).innerHTML = rowObjects[i].counter;
      } else {
    ...
    db.collection('dailyChecking')
      ...
      .then((doc) => {
        ...
        for (let i = 0; i < tableRows.length; i++) {
          rowObjects[i + 1].id = Number.parseInt(tableRows[i].id.substring(tableRows[i].id.indexOf('-') + 1));
          rowObjects[i + 1].color = tableRows[i].children[0].style.backgroundColor;
          ...
          rowObjects[i + 1].counter = Number.parseInt(tableRows[i].children[4].innerHTML);
      ...
      .then(function (data) {
        ...
        for (let i = 0; i < tableRows.length; i++) {
          ...
          if (gameTableArr[i].timestamp) {
            timeOfLastCheck = new Date(gameTableArr[i].timestamp * 1000).toTimeString();
          }
          ...
          tableRows[i].children[7].innerHTML = timeOfLastCheck
            ...
            .substring(0, timeOfLastCheck.substring(0, timeOfLastCheck.indexOf(' ')).length - 3);
...
setInterval(function () {
  // DO NOT TOUCH or the DOM breaks plus it COSTS money to request data!
...
setInterval(function() {
  ...
  if (hours === 10 && minutes === 0 && day > 0 && day < 6) {
    alarmAlert('Blackjack Lounge 4 should be open!');
  } else if (hours === 15 && minutes === 0) {
    alarmAlert('Zurich Landbased Ro should be open!');
  } else if (hours === 16 && minutes === 0 && day > 0 && day < 6) {
    alarmAlert('Blackjack Lounge 6 should be open!');
  } else if (hours === 17 && minutes === 0) {
    alarmAlert('Blackjack Italiano 2 should be open!');
  } else if (hours === 16 && minutes === 0) {
    alarmAlert('Reapeat after me: "FUUUUUfffikon!"');
  } else if (hours === 22 && minutes === 0) {
    alarmAlert('Payback Mystery Promo!\nCheck:\n * Animations\n * BGM SFX\n * Promo Message');
  }
  ...
  if (dailyCheckingUser) {
    ...
    db.collection('dailyChecking')
      ...
      .then((doc) => {
        ...
        doc.data().shift === 'long' ? (longShift.checked = true) : (longShift.checked = false);
    ...
    db.collection('dailyChecking')
      ...
      .then((doc) => {
        ...
        let deleteObject = rowObjects.find(
          (value) => Number.parseInt(value.id) === Number.parseInt(row.id.substring(row.id.indexOf('-') + 1)),
        );
...
const updateCounterAndOptions = function (event) {
  ...
  if (eventKey !== 'Shift' && event.type !== 'mouseover' && event.type !== 'mouseout' && event.type !== 'click') {
  ...
  if (target.classList.contains('highlight-this')) {
    let indexID = target.id.substring(target.id.indexOf('-') + 1),
      ...
      targetNumber = document.querySelector(`#target-${indexID}`) || menuToggleBtn,
  ...
  if (
    (target.name === 'table' && event.type === 'keyup' && eventKey.length === 1) ||
    (target.name === 'table' && eventKey === 'Backspace')
  ) {
    gameTableNames.innerHTML = '';
    tablesDB.forEach((value) => {
      if (optionArray.length < 10 && value.toLowerCase().startsWith(target.value.toLowerCase())) {
    ...
    tablesDB.forEach((value) => {
      if (optionArray.length < 10 && value.toLowerCase().includes(target.value.toLowerCase())) {
    ...
    (target.name === 'casino' && event.type === 'keyup' && eventKey.length === 1) ||
  ...
  } else if (target.classList.contains('submitButton') && event.type === 'click') {
    target.setAttribute('disabled', 'disabled');
    ...
    db.collection('dailyChecking')
      ...
      .then((doc) => {
        if (tableName != '' && platform != '' && casino != '') {
          ...
          let time = new Date().toTimeString().substring(0, new Date().toTimeString().indexOf(' '));

          rowObjects.forEach((object) => {
            if (Number.parseInt(object.id) === Number.parseInt(target.id)) {
              if (object.id > 0) {
                object.color = document.getElementById(`format-${object.id}`).style.backgroundColor;
              }
          ...
          let rowObjects = doc.data().rowObjects;
          let update = rowObjects[rowObjects.findIndex((obj) => Number.parseInt(obj.id) === Number.parseInt(target.id))];
          ...
          rowObjects[rowObjects.findIndex((obj) => Number.parseInt(obj.id) === Number.parseInt(target.id))] = update;
          db.collection('dailyChecking')
            .doc(userUID)
            .update({
              rowObjects: rowObjects,
    ...
    db.collection('dailyChecking')
      ...
      .then((doc) => {
        ...
        let newTracking = tracking.filter((item) => item.when.seconds > time7daysAgo);
      ...
      .then(([rowObjects, templateItems]) => {
        ...
        templateItems.forEach((item) => {
          for (let i = 0; i < rowObjects.length; i++) {
            ...
            } else if (i === rowObjects.length - 1 && item.name !== rowObjects[i].name) {
              newRowObjects.push({
                name: item.name,
                target: item.target,
                id: id,
    ...
    for (let i = 0; i < newTableRowOrder.length; i++) {
      ...
      tableRows[i].children[0].style.backgroundColor = newTableRowOrder[i].children[0].style.backgroundColor;
      ...
      tableRows[i].children[4].textContent = newTableRowOrder[i].children[4].textContent;
      ...
      tableRows[i].children[7].textContent = newTableRowOrder[i].children[7].textContent;
    }
    ...
    db.collection('dailyChecking')
      ...
      .then((doc) => {
        ...
        for (let i = 0; i < tableRows.length; i++) {
          rowObjects[i + 1].id = tableRows[i].id.substring(tableRows[i].id.indexOf('-') + 1);
          rowObjects[i + 1].color = tableRows[i].children[0].style.backgroundColor;
    ...
    let currentAmount = 0;
    allCounters.forEach((item) => (currentAmount += Number.parseInt(item.innerHTML)));
    ...
    if (currentAmount > 0) {
      db.collection('dailyChecking')
        ...
        .then((doc) => {
          ...
          rowObjects[rowObjects.findIndex((object) => Number.parseInt(object.id) === Number.parseInt(lastTracked.id))].counter--;
          ...
          db.collection('dailyChecking')
            ...
            .then(() => {
              let matchedCounter = document.getElementById(`counter-${lastTracked.id}`);
  ...
  } else if (target.classList.contains('row-format') && event.type === 'click') {
  ...
  } else if (target.classList.contains('color-option') && event.type === 'click') {
    ...
    db.collection('dailyChecking')
      ...
      .then((doc) => {
        let rowObjects = doc.data().rowObjects;
        let rowToFormatID = target.closest('.row-format').id.substring(target.closest('.row-format').id.indexOf('-') + 1);

        // Finding the index of the rowObject where the ID matches client side formatted rows ID
        let rowToFormat = rowObjects[rowObjects.findIndex((obj) => Number.parseInt(obj.id) === Number.parseInt(rowToFormatID))];
...
const clearInputValues = function (event) {
  if (
    (event.target.classList.contains('platform-name') && event.target.id !== 'platform-0') ||
    event.target.id === 'table-0' ||
    (event.target.classList.contains('casino-name') && event.target.id !== 'casino-0')
  ) {
    event.target.value = '';
  }
};
...
const moveRows = function (event) {
  const removeAddedClassesAndTempRow = (id) => {
    ...
    while (rowsViewed < tableRows.length) {
      if (document.getElementById('row-' + iterations)) {
        document.getElementById('row-' + iterations).classList.remove('move-rows-target-below');
        rowsViewed++;
      }
      iterations++;
    }
  ...
  if (event.type === 'dragstart') {
    ...
    !event.target.closest('.table-row').classList.contains('move-rows-target-above')
  ) {
    let placeholderLocation = 'above';

    event.target.closest('.table-row').classList.contains('move-rows-target-below')
...
themeToggle.onclick = function () {
  if (themeSwitch.checked) {
    ...
    document.cookie = 'color-schema=css/light.css;max-age=695520‬;secure;samesite=strict';
  } else {
    theBall.style.transitionDuration = '0.2s';
    styleSheet.href = 'css/dark.css';
    document.cookie = 'color-schema=css/dark.css;max-age=695520;secure;samesite=strict';
  }
};

menuToggleBtn.onclick = function () {
  hiddenMenu.style.visibility = hiddenMenu.style.visibility != 'visible' ? 'visible' : 'hidden';
  menuToggleBtn.blur();
};
